...
const COPY = {
  en: {
    ...
    notificationText: "Use the Notifications control for time, type, quiet days and per-child toggles.",
  ...
  pt: {
    ...
    notificationText: "Usa o controlo de Notificações para hora, tipo, dias silenciosos e crianças.",
...
export default function SettingsHubLayer() {
  ...
  useEffect(() => {
    loadData();
    const openSettings = () => { setOpen(true); setSection("menu"); loadData(); };
    window.addEventListener("zommy:open-settings-hub", openSettings);
    const { data: { subscription } } = supabase.auth.onAuthStateChange(() => loadData());
    return () => {
      window.removeEventListener("zommy:open-settings-hub", openSettings);
  ...
  const menuItems = [
    ...
    ["export", copy.exportData, `${entries.length} memories`],
    ["delete", copy.deleteAccountData, "Permanent"],
    ["storage", copy.storageUsage, formatBytes(estimatedStorage)],
    ["privacy", copy.privacy, "Private account data"],
    ["sharing", copy.familySharing, copy.comingSoon],
    ["notifications", copy.notificationSchedule, "Open controls"],
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
        <div style={{ display: "flex", gap: 11, alignItems: "center", minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.52)", marginTop: 2 }}>{profile.birthdate} · {entryCountByProfile[profile.id] || 0} memories</div>
          </div>
        </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1600, background: "#101418", color: "#fff", overflowY: "auto", fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ maxWidth: 480, minHeight: "100dvh", margin: "0 auto", padding: "18px 16px calc(26px + env(safe-area-inset-bottom, 0px))" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 16 }}>
          <h1 style={{ fontFamily: "Lora, Georgia, serif", fontSize: 29, lineHeight: 1.1, fontWeight: 650 }}>{section === "menu" ? copy.title : menuItems.find(([id]) => id === section)?.[1]}</h1>
          <button onClick={() => section === "menu" ? setOpen(false) : setSection("menu")} style={ghostButton()}>{section === "menu" ? copy.close : "←"}</button>
        </header>

        {toast && <div style={{ background: "#fff", color: "#111", borderRadius: 14, padding: "10px 12px", fontSize: 13, fontWeight: 800, marginBottom: 12 }}>{toast}</div>}
        ...
        {section === "export" && <Panel><p>{entries.length} memories · {profiles.length} profiles</p><button onClick={exportData} style={primaryButton()}>{copy.exportData}</button></Panel>}
...
const ghostButton = () => ({ border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.72)", borderRadius: 999, minWidth: 42, minHeight: 42, padding: "8px 12px", fontSize: 14, cursor: "pointer" });
