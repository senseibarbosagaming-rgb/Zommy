...
export default function Zommy() {
  ...
  const saveEntry = () => {
    ...
    const entry = { date: logDate, photo: logPhoto, note: logNote, id: Date.now() };
    const prev = state.entries[activeId] || [];
    update({
      ...state,
      entries: {
        ...state.entries,
        [activeId]: [...prev.filter((e) => e.date !== logDate), entry]
          .sort((a, b) => b.date.localeCompare(a.date)),
      },
    });
    setLogPhoto(null); setLogPreview(null); setLogNote(""); setLogDate(today());
    showToast("Saved ✓");
    setView("timeline");
  };
              ...
              {state.profiles.map((p) => {
                ...
                return (
                  <div key={p.id} className="c" style={{ ...s.card, borderColor: p.color + "33" }}>
                    ...
                    <div style={s.pCount}>{pe.length} {pe.length === 1 ? "day" : "days"} logged</div>
                    <div style={s.pActions}>
                      <button className="b" style={{ ...s.btn1, background: p.color }} onClick={() => goLog(p.id)}>+ add day</button>
        ...
        {view === "log" && active && (
          <div className="f" style={s.sec}>
            <div style={s.secHead}>
              ...
              <div>
                <h2 style={s.secTitle}>Log a day for {active.name}</h2>
                {getAge(active.birthdate, logDate) && (
                  <div style={{ color: active.color, fontSize: 12, marginTop: 3, fontStyle: "italic" }}>
          ...
          <div className="f" style={s.sec}>
            <div style={s.secHead}>
              ...
              <button className="b" style={{ ...s.btn1, background: active.color, marginTop: 10 }}
                onClick={() => goLog(activeId)}>+ add day</button>
            </div>
            {sorted.length === 0 ? (
                      ...
                      {sel ? (
                        <div style={s.cmpCard}>
                          ...
                          <div style={{ padding: "8px 10px 4px" }}>
                            <div style={{ fontFamily: "'Lora',serif", fontSize: 12, fontWeight: 600, color: cp?.color }}>{formatDate(sel.date)}</div>
                            {cp && getAge(cp.birthdate, sel.date) && (
                              <div style={{ fontSize: 10, color: "#bbb", fontStyle: "italic", marginTop: 2 }}>{getAge(cp.birthdate, sel.date)}</div>
                            )}
                            {sel.note && <div style={{ fontSize: 11, color: "#777", marginTop: 3, lineHeight: 1.4 }}>{sel.note}</div>}
                          </div>
                          <button className="b" style={s.chgBtn} onClick={() => set(null)}>change</button>
                        </div>
                      ) : (
                        <div style={s.cmpPicker}>
                          <p style={{ color: "#bbb", fontSize: 12, marginBottom: 8 }}>Pick a day</p>
                          {cpe.length === 0 && <p style={{ color: "#ddd", fontSize: 11 }}>No entries yet</p>}
                          <div style={s.cmpList}>
                            {cpe.map((e) => (
                              <button key={e.id} className="b" onClick={() => set(e)}
                                ...
                                <span style={{ fontSize: 11, color: "#555", textAlign: "left", lineHeight: 1.4 }}>
                                  {formatDate(e.date)}
                                  {cp && getAge(cp.birthdate, e.date) && (
                                    <span style={{ display: "block", color: "#bbb", fontSize: 10 }}>{getAge(cp.birthdate, e.date)}</span>
                                  )}
                                </span>
                              </button>
                            ))}
      ...
      <nav style={s.nav}>
        {[{ id: "home", icon: "⌂", lbl: "Home" }, { id: "log", icon: "+", lbl: "Log" },
          ...
          <button key={n.id} className="b"
            ...
            <span style={{ fontSize: 10, marginTop: 2 }}>{n.lbl}</span>
          </button>
        ))}
      </nav>
    </div>
...
const s = {
  ...
  back: { background: "none", border: "none", fontSize: 13, color: "#999", fontFamily: "'DM Sans',sans-serif", padding: "4px 0", cursor: "pointer" },
  main: { flex: 1, paddingBottom: 80, overflowY: "auto" },
  toast: { position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: "#1a1a1a", color: "#fff", padding: "10px 20px", borderRadius: 100, fontSize: 13, zIndex: 100, whiteSpace: "nowrap" },

  home: { padding: "24px 20px 20px" },
  tagline: { fontFamily: "'Lora',serif", fontStyle: "italic", color: "#aaa", fontSize: 14, marginBottom: 24, letterSpacing: "-0.2px" },
  ...
  pAge: { fontSize: 11, fontWeight: 500 },
  pCount: { fontSize: 11, color: "#ccc" },
  pActions: { display: "flex", flexDirection: "column", gap: 6, width: "100%", marginTop: 6 },
  btn1: { padding: "8px 14px", borderRadius: 100, border: "none", color: "#fff", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", textAlign: "center" },
  btn2: { padding: "8px 14px", borderRadius: 100, border: "1.5px solid", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", textAlign: "center" },
  addBtn: { width: "100%", padding: "12px", background: "#fff", border: "1.5px dashed #ddd", borderRadius: 12, fontSize: 13, color: "#888", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", marginBottom: 10 },
  cmpBtn: { width: "100%", padding: "12px", background: "#fff", border: "1.5px solid #EDEAE4", borderRadius: 12, fontSize: 13, color: "#555", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" },
  ...
  tab: { padding: "6px 14px", borderRadius: 100, border: "1.5px solid", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontWeight: 500 },
  lbl: { fontSize: 11, color: "#aaa", letterSpacing: "0.6px", textTransform: "uppercase", fontWeight: 500, marginBottom: -5 },
  inp: { padding: "11px 14px", borderRadius: 10, border: "1.5px solid #EDEAE4", fontSize: 14, background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans',sans-serif", width: "100%" },
  ...
  remBtn: { position: "absolute", top: 10, right: 10, background: "rgba(0,0,0,.5)", color: "#fff", border: "none", borderRadius: 100, padding: "5px 12px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" },
  ta: { padding: "11px 14px", borderRadius: 10, border: "1.5px solid #EDEAE4", fontSize: 14, fontFamily: "'DM Sans',sans-serif", color: "#1a1a1a", background: "#fff", resize: "none", lineHeight: 1.6, width: "100%" },

  empty: { display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "50px 20px", color: "#bbb", fontSize: 14 },
  tl: { display: "flex", flexDirection: "column", gap: 12 },
  eCard: { background: "#fff", borderRadius: 14, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,.05)" },
  ePhoto: { width: "100%", maxHeight: 250, objectFit: "cover", display: "block" },
  eInfo: { padding: "10px 14px 12px" },
  eDate: { fontFamily: "'Lora',serif", fontSize: 14, fontWeight: 600 },
  eAge: { fontSize: 11, color: "#bbb", marginTop: 2, fontStyle: "italic" },
  eNote: { fontSize: 13, color: "#777", marginTop: 5, lineHeight: 1.5 },

  cmpGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 },
  cmpCol: { display: "flex", flexDirection: "column", gap: 8 },
  cmpLbl: { fontSize: 10, color: "#bbb", textTransform: "uppercase", letterSpacing: "0.6px", fontWeight: 500 },
  ...
  chgBtn: { background: "none", border: "none", color: "#bbb", fontSize: 11, cursor: "pointer", padding: "4px 10px 8px", fontFamily: "'DM Sans',sans-serif" },
